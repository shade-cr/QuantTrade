...
def test_no_fire_after_early_actual_announcement(monkeypatch):
    """DA medium: if the real announcement lands EARLY (inside the expected
    window), no bar at/after the session that knows it may fire — the
    pre-announcement premium no longer exists."""
    dates = [pd.Timestamp("2010-01-15 12:00", tz="UTC") + pd.Timedelta(days=91 * i)
             for i in range(4)]
    # 5th announcement arrives 6 days EARLY (BMO), landing inside the
    # expected window for event 5.
    dates.append(dates[-1] + pd.Timedelta(days=85))
    _install_announcements(monkeypatch, dates)
    sig = epp.signal(_ohlcv(periods=500), pd.DataFrame(), {"_current_asset": "TEST"})
    early_known = pd.Timestamp(dates[-1]).tz_localize(None).normalize()

    # The expectation generated by announcement 4 (known_asof = dates[3])
    # has its window straddling the EARLY actual announcement. No fire from
    # that window may occur at/after the session that knows the actual.
    from pipeline.earnings_events import expected_announcement_schedule
    sched = expected_announcement_schedule(pd.DatetimeIndex(dates))
    row5 = sched[sched["known_asof"] == dates[3]]
    exp5 = pd.Timestamp(row5.index[0]).tz_localize(None).normalize()
    w_start = exp5 - pd.tseries.offsets.BusinessDay(epp.ENTRY_DAYS_BEFORE)
    w_end = exp5 - pd.tseries.offsets.BusinessDay(1)
    for ts in sig[sig == 1].index:
        d = ts.tz_localize(None).normalize()
        if w_start <= d <= w_end:
            assert d < early_known, "fired after the actual announcement was known"


def test_amendments_do_not_spawn_windows(monkeypatch):
    """DA high #2: an 8-K/A five days after a real announcement must not
    change the signal at all (filtered before scheduling)."""
    clean = [pd.Timestamp("2010-01-15 12:00", tz="UTC") + pd.Timedelta(days=91 * i)
             for i in range(5)]
    with_amendment = sorted(clean + [clean[2] + pd.Timedelta(days=5)])
    ohlcv = _ohlcv(periods=450)
    _install_announcements(monkeypatch, clean)
    base = epp.signal(ohlcv, pd.DataFrame(), {"_current_asset": "TEST"})
    _install_announcements(monkeypatch, with_amendment)
    amended = epp.signal(ohlcv, pd.DataFrame(), {"_current_asset": "TEST"})
    pd.testing.assert_series_equal(base, amended)
